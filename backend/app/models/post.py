...
class Post(Base):
    ...
    subtitle: Mapped[Optional[str]] = mapped_column(String(300)) # mały tytuł
    slug: Mapped[str] = mapped_column(String(220), unique=True, index=True)
    # content: Mapped[str] = mapped_column(Text) # pełny tekst
    tags: Mapped[str] = mapped_column(String(400), default="") # np. "Trening,Wiedza"
    author_id: Mapped[int] = mapped_column(Integer, index=True)
    author_name: Mapped[str] = mapped_column(String(120)) # zapisujemy wygodnie
